...
async function main() {
  ...
  for await (const entry of stream) {
    ...
    const packageName = Math.floor(Math.random() * 101);

    body += `<p><b>Service: ${packageName}</b></p><p>Covered ${getSummary(
      lines
    )}.</p><br><br>`;
  }
  ...
  const productionCoveragefile = fs.readFileSync(
    core.getInput("production-lcov-path")
  );
  ...
  const body = `<p><b>Service: ${packageName}</b></p><p>Covered ${getSummary(
    lines
  )}. Coverage ${productionReport} production.</p>`;*/
