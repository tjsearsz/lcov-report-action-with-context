...
async function main() {
  ...
  const coverallsResult = JSON.parse(core.getInput('coveralls-result'));
  const body = `<p><b>Service: ${packageName}</b></p><p>Covered ${getSummary(lines)} lines</p><p>See More on <a href="${coverallsResult.url()}">Coveralls.</a>`;
